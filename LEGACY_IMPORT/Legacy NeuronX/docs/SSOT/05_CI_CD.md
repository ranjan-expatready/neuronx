...
### Spec Artifact Enforcement (PHASE 8A)

**Trigger Conditions**: PR requires spec when ANY is true:
- ≥3 files changed (excluding docs-only)
- Touches auth/security/permissions
- Touches CI/governance/agent_runtime
- Touches database schema/migrations

**Passing Criteria**:
- Spec artifact exists in `.factory/docs/*.md`
- PR body references spec with `Spec: .factory/docs/YYYY-MM-DD-*.md`
- OR PR declares "Spec N/A" with justification for docs-only/single-file

**Reversible**: Set `ENFORCE_SPEC_ARTIFACT=false` in GitHub vars/CI env

详见: `.factory/docs/2026-01-17-phase-8b-mechanical-spec-mode-enforcement-via-ci-reversible.md`
