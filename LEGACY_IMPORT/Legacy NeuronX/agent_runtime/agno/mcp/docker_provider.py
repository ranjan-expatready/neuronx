"""
Docker MCP Provider - Phase 4B

Provides a guarded adapter to the Docker MCP server.
"""
...
class DockerMCPProvider:
    """Docker MCP provider routed through the MCP bridge."""
    ...
    def call(self, action: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.server_mode == "noop":
            response["error"] = "Docker MCP noop mode: server not available"
            response["meta"] = self._base_meta(normalized_action, params, server_available=False)
            return response

        if not self.command:
            response["error"] = "No Docker MCP command configured"
        ...
        if not cmd_available:
            response["error"] = f"Docker MCP command not found: {cmd0}"
            response["meta"] = meta
            return response
