"""
Filesystem MCP Provider - Phase 4B

Provides a guarded adapter to the Filesystem MCP server.
"""
...
class FilesystemMCPProvider:
    """Filesystem MCP provider routed through the MCP bridge."""
    ...
    def call(self, action: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute a Filesystem MCP action.
        """
        ...
        if self.server_mode == "noop":
            response["error"] = "Filesystem MCP noop mode: server not available"
            response["meta"] = self._base_meta(normalized_action, params, server_available=False)
            return response

        if not self.command:
            response["error"] = "No Filesystem MCP command configured"
        ...
        if not cmd_available:
            response["error"] = f"Filesystem MCP command not found: {cmd0}"
            response["meta"] = meta
            return response
