"""
Linear MCP Provider - Phase 4B

Provides a guarded adapter to the Linear MCP server.
"""
...
class LinearMCPProvider:
    """Linear MCP provider routed through the MCP bridge."""
    ...
    def call(self, action: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.server_mode == "noop":
            response["error"] = "Linear MCP noop mode: server not available"
            response["meta"] = self._base_meta(normalized_action, params, server_available=False)
            return response

        if not self.command:
            response["error"] = "No Linear MCP command configured"
        ...
        if not api_key:
            response["error"] = "LINEAR_API_KEY not set"
        ...
        if not cmd_available:
            response["error"] = f"Linear MCP command not found: {cmd0}"
            response["meta"] = meta
            return response
