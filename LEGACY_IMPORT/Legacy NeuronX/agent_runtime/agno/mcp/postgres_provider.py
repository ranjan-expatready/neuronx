"""
PostgreSQL MCP Provider - Phase 4B

Provides a guarded adapter to the PostgreSQL MCP server.
"""
...
class PostgresMCPProvider:
    """PostgreSQL MCP provider routed through the MCP bridge."""
    ...
    def call(self, action: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute a PostgreSQL MCP action.
        """
        ...
        if self.server_mode == "noop":
            response["error"] = "PostgreSQL MCP noop mode: server not available"
            response["meta"] = self._base_meta(normalized_action, params, server_available=False)
            return response

        if not self.command:
            response["error"] = "No PostgreSQL MCP command configured"
            response["meta"] = self._base_meta(normalized_action, params, server_available=False, command_available=False)
            return response

        # Check for Database URL (should be in command args or env)
        # The official server usually takes the connection string as an argument
        # We can also check for DATABASE_URL env var if the server supports it, 
        # but typically it's passed as an arg.
        # Here we just assume the command is configured correctly with the URL.
        ...
        if not cmd_available:
            response["error"] = f"PostgreSQL MCP command not found: {cmd0}"
            response["meta"] = meta
            return response
