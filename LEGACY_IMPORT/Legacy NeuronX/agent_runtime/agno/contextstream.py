...
class ContextStreamClient:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.store_endpoint = os.getenv('CONTEXTSTREAM_STORE_ENDPOINT') or self.config.get('store_endpoint', '/store')
    ...
    def store(self, record: Dict[str, Any]) -> None:
        """
        Store a memory record using urllib.request (stdlib only).
        ...
        # Warning about storage contract
        print("⚠️  ContextStream store: No storage contract found in repo documentation")
        print(f"   Using default endpoint: {self.store_endpoint}")
        print("   If this endpoint is incorrect, set CONTEXTSTREAM_STORE_ENDPOINT environment variable")

        try:
            from urllib.request import Request, urlopen
            from urllib.error import URLError, HTTPError

            url = f"{self.url}{self.store_endpoint}"
            payload = json.dumps({
                'record': record,
                'project': self.project
            }).encode('utf-8')
            ...
            with urlopen(req, timeout=10) as response:
                if response.code < 200 or response.code >= 300:
                    raise RuntimeError(f"ContextStream storage failed: HTTP {response.code}")

            print(f"💾 ContextStream: Stored memory '{record.get('summary', 'unknown')[:50]}...'")
        except HTTPError as e:
            raise RuntimeError(f"ContextStream storage HTTP error (endpoint: {self.store_endpoint}): {e.code} - {e.reason}")
        except URLError as e:
            raise RuntimeError(f"ContextStream storage connection error (endpoint: {self.store_endpoint}): {e.reason}")
        except Exception as e:
            raise RuntimeError(f"ContextStream storage failed (endpoint: {self.store_endpoint}): {e}")
