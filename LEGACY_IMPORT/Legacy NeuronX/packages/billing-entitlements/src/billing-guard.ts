...
export class BillingGuard {
  ...
  async checkExecutionEntitlement(
    ...
    try {
      ...
      this.logger.error(`Billing check failed: ${(error as Error).message}`, {
        tenantId,
        correlationId,
      ...
      return {
        allowed: false,
        reason: `Billing system error: ${(error as Error).message}`,
        enforcementMode: (this.evaluator as any)['enforcementMode'], // Accessing private property temporarily for fallback
      };
    }
  }
  ...
  async recordExecutionUsage(
    ...
    try {
      ...
      this.logger.error(`Failed to record execution usage: ${(error as Error).message}`, {
        tenantId,
        correlationId,
        error: (error as Error).stack,
      });
