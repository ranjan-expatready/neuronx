...
export class EntitlementEvaluator {
  ...
  async checkEntitlement(
    ...
    // Log correlationId for tracking (avoid unused variable)
    // console.log(`Checking entitlement for ${tenantId} with correlation ${correlationId}`);
    correlationId.length; // dummy usage
    ...
    try {
      ...
      return this.createDecision(
        ...
        `Entitlement check failed: ${(error as Error).message}`,
        {
          enforcementMode: defaultEnforcementMode,
        }
      );
