...
export class DecisionContextBuilderImpl implements DecisionContextBuilder {
  create(
    ...
    return {
      ...
      tenantId,
      executionCommand,
      correlationId: partial.correlationId ?? `dc-${Date.now()}`,
      evidenceSoFar: (partial.evidenceSoFar as any) ?? [],
      dealValue: partial.dealValue,
      customerRiskScore: partial.customerRiskScore,
      // stageDurationMinutes: (partial as any).stageDurationMinutes,
