...
class Logger {
  ...
  debug(message: any, ...optionalParams: any[]) { console.debug(`[${this.context}]`, message, ...optionalParams); }
  info(message: any, ...optionalParams: any[]) { console.info(`[${this.context}]`, message, ...optionalParams); }
}
...
import type { DecisionContextBuilder } from './decision-context';
import type { RiskGate } from './risk-gate';
import type { ActorSelector } from './actor-selector';
import type { VoiceModeSelector } from './voice-mode-selector';
import type { DecisionPolicyResolver } from './policy/decision-policy.resolver';
...
export class DecisionEngineImpl implements DecisionEngine {
  ...
  private logDecision(
    ...
    const _auditEvent: Omit<DecisionAuditEvent, 'eventId' | 'timestamp'> = {
    ...
    // TODO: Emit audit event to event bus when available
    // this.eventBus.emit('decision.audit', auditEvent);
