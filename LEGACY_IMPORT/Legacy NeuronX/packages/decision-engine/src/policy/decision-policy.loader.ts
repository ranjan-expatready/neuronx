...
export class DecisionPolicyLoader implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to load decision policy: ${err.message}`);
  ...
  private async loadAndValidatePolicy(): Promise<PolicyLoadResult> {
    ...
    try {
      ...
      const err = error instanceof Error ? error : new Error(String(error));
      throw new Error(`Failed to read decision policy file: ${err.message}`);
    }
    ...
    try {
      ...
      const err = error instanceof Error ? error : new Error(String(error));
      throw new Error(`Failed to parse decision policy YAML: ${err.message}`);
    }
