...
export class GhlDriftDetectionService {
  ...
  private async getSnapshotsForComparison(
    ...
    if (request.beforeSnapshotId) {
      beforeSnapshot = await (this.snapshotService as any)
        .getSnapshotStorage()
      ...
      // Use latest - 1
      const snapshots = await (this.snapshotService as any).getSnapshotStorage().query({
        tenantId: request.tenantId,
        snapshotType: request.snapshotType,
        limit: 2,
      });
    ...
    if (request.afterSnapshotId) {
      afterSnapshot = await (this.snapshotService as any)
        .getSnapshotStorage()
  ...
  async getDriftStats(tenantId: string): Promise<{
    ...
    const criticalDriftCount = drifts.filter(d => d.summary.hasCriticalChanges).length;
