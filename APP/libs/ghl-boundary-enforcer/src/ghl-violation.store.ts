...
export class PrismaGhlViolationStore implements GhlViolationStore {
  ...
  async getViolationSummary(
    ...
    const summary = {
      ...
      mostRecentViolation:
        violations.length > 0
          ? new Date(Math.max(...violations.map((v: any) => v.createdAt.getTime())))
          : undefined,
    };

    // Count by severity
    violations.forEach((v: any) => {
      summary.violationsBySeverity[v.severity as ViolationSeverity] =
        (summary.violationsBySeverity[v.severity as ViolationSeverity] || 0) + 1;
    });

    // Count by type
    violations.forEach((v: any) => {
      summary.violationsByType[v.violationType as ViolationType] =
        (summary.violationsByType[v.violationType as ViolationType] || 0) + 1;
    });

    // Count by entity type
    violations.forEach((v: any) => {
      summary.violationsByEntityType[v.entityType as EntityType] =
        (summary.violationsByEntityType[v.entityType as EntityType] || 0) + 1;
    });
