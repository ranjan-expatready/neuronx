...
export class VoiceModeSelectorImpl implements VoiceModeSelector {
  ...
  isVoiceAllowed(
    ...
    // Safely check evidence type since it might be a string or object
    const negativeEvidence = context.evidenceSoFar.filter(e => {
      const evidenceStr = typeof e === 'string' ? e : JSON.stringify(e);
      return (
        evidenceStr.includes('failed') ||
        evidenceStr.includes('disqualified') ||
        evidenceStr.includes('angry') ||
        evidenceStr.includes('complaint')
      );
    });
    ...
    if (negativeEvidence.length > 0) {
      return {
        allowed: false,
        reason: `Negative evidence suggests human voice intervention required`,
      };
    }
  ...
  private requiresScriptedMode(
    ...
    // Safe check for actionType as it might not exist on all ExecutionCommand types
    const cmd = context.executionCommand as any;
    if (
      cmd.actionType &&
      ['qualification_call', 'negotiation'].includes(cmd.actionType)
    ) {
      return true;
    }

    // Evidence suggesting caution
    const cautionEvidence = context.evidenceSoFar.filter(e => {
      const evidenceStr = typeof e === 'string' ? e : JSON.stringify(e);
      return (
        evidenceStr.includes('hesitant') ||
        evidenceStr.includes('uncertain') ||
        evidenceStr.includes('followup')
      );
    });
  ...
  private allowsConversationalMode(
    ...
    const cmd = context.executionCommand as any;
    if (
      cmd.actionType === 'followup' ||
      cmd.actionType === 'contact_attempt'
    ) {
      return true;
    }

    // Positive evidence suggests conversational is fine
    const positiveEvidence = context.evidenceSoFar.filter(e => {
      const evidenceStr = typeof e === 'string' ? e : JSON.stringify(e);
      return (
        evidenceStr.includes('engaged') ||
        evidenceStr.includes('interested') ||
        evidenceStr.includes('responsive')
      );
    });
