...
export class DriftStorageService implements DriftStorage {
  ...
  async store(result: DriftDetectionResult): Promise<void> {
    try {
      await (this.prisma as any).ghlDrift.create({
        data: {
  ...
  async retrieve(driftId: string): Promise<DriftDetectionResult | null> {
    try {
      const record = await (this.prisma as any).ghlDrift.findUnique({
        where: { driftId },
      });
  ...
  async query(
    ...
    try {
      ...
      const records = await (this.prisma as any).ghlDrift.findMany({
        where,
  ...
  async getLatestDrift(
    ...
    try {
      const record = await (this.prisma as any).ghlDrift.findFirst({
        where: {
          tenantId,
          snapshotType,
        },
  ...
  async getStats(tenantId?: string): Promise<{
    ...
    try {
      ...
      const [totalCount, typeStats, severityStats, dateRange] =
        await Promise.all([
          (this.prisma as any).ghlDrift.count({ where }),
          (this.prisma as any).ghlDrift.groupBy({
            by: ['snapshotType'],
            where,
            _count: { snapshotType: true },
          }),
          (this.prisma as any).ghlDrift.findMany({
            where,
            select: { summary: true },
          }),
          (this.prisma as any).ghlDrift.aggregate({
            where,
      ...
      severityStats.forEach((record: any) => {
