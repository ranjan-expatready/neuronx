...
export { SnapshotType } from '@neuronx/ghl-snapshots';
...
export type DriftDetection = DriftChange;
...
export function classifyDriftCategory(
  _type: SnapshotType,
  _path: string,
  _changeType: DriftChangeType
): DriftCategory {
  return DriftCategory.CONFIG_DRIFT;
}

export function assessDriftSeverity(
  _category: DriftCategory,
  _changeType: DriftChangeType,
  _path: string,
  _entityType: string
): DriftSeverity {
...
export function getOverallSeverity(_changes: DriftChange[]): DriftSeverity {
  return DriftSeverity.MEDIUM;
}

export function hasBreakingChanges(_changes: DriftChange[]): boolean {
  return false;
}

export function requiresReview(
  _changes: DriftChange[],
  _config?: DriftDetectionConfig
): boolean {
  return false;
}

export interface DriftAnalysisResult extends Omit<DriftDetectionResult, 'summary'> {
  analysisId?: string;
  analysisDurationMs?: number;
  summary: DriftDetectionResult['summary'] & {
