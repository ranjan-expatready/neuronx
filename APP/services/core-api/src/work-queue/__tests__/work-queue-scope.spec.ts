...
describe('WorkQueueService Scope Filtering', () => {
  ...
  describe('getWorkQueue scope filtering', () => {
    ...
    // WI-037 Hardening: Verify creation/transformation logic from Routing Failure
    it('should generate approval_required work item for high-value qualified opportunity', async () => {
      // Mock authority context for operator
      mockOrgAuthority.getAuthorityContext.mockResolvedValue({
        tenantId: 'tenant_1',
        memberId: 'member_1',
        userId: 'user_123',
        roleAssignments: [{
          id: 'ra_1', tenantId: 'tenant_1', memberId: 'member_1', role: 'OPERATOR',
      ...
      const getAccessibleScopesSpy = vi.spyOn(workQueueService as any, 'getAccessibleScopes');
      getAccessibleScopesSpy.mockResolvedValue({ teams: ['team_1'], agencies: [] });
      ...
      expect(result.items[0].type).toBe('approval_required');
      expect(result.items[0].reason).toContain('High-value');
    ...
    it('should include unassigned opportunities for agency admins', async () => {
      ...
      // The actual query uses agencyId filtering which covers both team-assigned and unassigned within the agency
      expect(mockPrisma.opportunity.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({
            OR: expect.arrayContaining([
              expect.objectContaining({ agencyId: { in: ['agency_1'] } }),
            ]),
          }),
        })
      );
