...
export class OutcomeTracker {
  private outcomeEvents: OutcomeEvent[] = new Map<string, OutcomeEvent[]>();
  ...
  private calculateVariantMetrics(
    ...
    const confidenceInterval95 = {
      conversionTimeMinutes: this.calculateConfidenceInterval(
        outcomes.map(o => o.conversionTimeMinutes).filter(Boolean) as number[],
        0.95
      ),
      totalInteractions: this.calculateConfidenceInterval(
        outcomes.map(o => o.totalInteractions),
        0.95
      ),
      complianceScore: this.calculateConfidenceInterval(
        outcomes.map(o => o.complianceScore).filter(Boolean) as number[],
        0.95
      ),
      totalCost: this.calculateConfidenceInterval(
        outcomes.map(o => o.totalCost).filter(Boolean) as number[],
        0.95
      ),
    };
  ...
  generateExperimentResults(
    ...
    const confidenceIntervals = {
      conversionRate: [avgConversionRate - 0.05, avgConversionRate + 0.05],
      timeToConversion: avgTimeToConversion
        ? [avgTimeToConversion - 10, avgTimeToConversion + 10]
        : [0, 0],
    };
