...
export class ComparisonEngine {
  ...
  analyzeExperimentResults(results: ExperimentResults): {
    ...
    const confidenceIntervals: Record<string, ConfidenceInterval> = {
      conversionRate: {
        ...
        lowerBound: results.confidenceIntervals.conversionRate[0],
        upperBound: results.confidenceIntervals.conversionRate[1],
        confidenceLevel: 0.95,
      },
      timeToConversion: {
        metric: 'timeToConversion',
        lowerBound: results.confidenceIntervals.timeToConversion[0],
        upperBound: results.confidenceIntervals.timeToConversion[1],
        confidenceLevel: 0.95,
      },
    };
  ...
  validateExperimentStatistics(results: ExperimentResults): {
    ...
    for (const [variantId, results] of Object.entries(results.variantResults)) {
      if (results.conversionRate < 0.01 || results.conversionRate > 0.95) {
        issues.push(
          `Extreme conversion rate detected for ${variantId}: ${(results.conversionRate * 100).toFixed(1)}%`
        );
