...
export abstract class BaseDriftComparator {
  ...
  protected createDriftDetection(
    ...
    const severity = assessDriftSeverity(
      category,
      changeType,
      diffPath,
      this.getEntityType()
    ...
    return {
      ...
      impactAssessment,
      metadata: {
        isBreakingChange: this.isBreakingChange(changeType, diffPath, severity),
        requiresReview: this.requiresReview(severity, category),
   ...
   */
  private getAffectedEntities(entityId: string, diffPath: string): string[] {
