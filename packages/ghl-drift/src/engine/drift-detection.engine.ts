...
export class DriftDetectionEngine {
  ...
  async analyzeDrift(
    ...
    try {
      ...
      const changesBySeverity = this.countBySeverity(detections);
      ...
      const result: DriftAnalysisResult = {
        ...
        totalEntities: Math.max(
          beforeSnapshot.metadata.recordCount,
          afterSnapshot.metadata.recordCount
        ),
      ...
      // Audit the analysis
      await this.auditDriftAnalysis(result, request.correlationId);
  ...
  private async getSnapshotsForComparison(
    ...
    if (request.beforeSnapshotId && request.afterSnapshotId) {
      ...
      [beforeSnapshot, afterSnapshot] = await Promise.all([
      ...
      const latestSnapshots = await this.getSnapshotsInWindow(
        ...
        this.config.comparisonWindowHours
      );
   ...
   */
  private recordMetrics(result: DriftAnalysisResult): void {
