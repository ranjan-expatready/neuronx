...
export class GhlSnapshotService {
  ...
  constructor(
    ...
    this.config = config;
    this.prisma = prisma;
    this.logger = new Logger(`GhlSnapshotService:${config.tenantId}`);
  }
  ...
  async getLatestSnapshot(tenantId: string): Promise<GhlSnapshotData | null> {
    const snapshot = await this.prisma.ghlSnapshot.findFirst({
      where: { tenantId },
  ...
  async getSnapshot(snapshotId: string): Promise<GhlSnapshotData | null> {
    const snapshot = await this.prisma.ghlSnapshot.findUnique({
      where: { id: snapshotId },
      include: {
        contacts: true,
        opportunities: true,
  ...
  async listSnapshots(
    ...
    const [snapshots, total] = await Promise.all([
      this.prisma.ghlSnapshot.findMany({
        where: { tenantId },
        orderBy: { pulledAt: 'desc' },
        take: limit,
        skip: offset,
      ...
      this.prisma.ghlSnapshot.count({ where: { tenantId } }),
    ]);
  ...
  async cleanupOldSnapshots(): Promise<number> {
    ...
    cutoffDate.setDate(
      cutoffDate.getDate() - this.config.snapshot.retentionDays
    );

    const result = await this.prisma.ghlSnapshot.deleteMany({
      where: {
  ...
  private async pullContacts(
    ...
    const { leads } = await this.adapter.listLeads(
  ...
  private async pullOpportunities(
    ...
    const { opportunities } = await this.adapter.listOpportunities(
      { limit: this.config.snapshot.maxRecordsPerSnapshot },
      context
    );
  ...
  private async pullUsers(context: AdapterContext): Promise<any[]> {
    const { users } = await this.adapter.listUsers(
