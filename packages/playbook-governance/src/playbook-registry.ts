...
export class PlaybookRegistry {
  ...
  retireVersion(
    ...
    const version = this.getVersion(playbookId, version, tenantId);
    if (!version) {
      return {
        success: false,
    ...
    if (version.state !== PlaybookVersionState.ACTIVE) {
      return {
        success: false,
        affectedOpportunities: 0,
        errors: [
          `Version is in ${version.state} state, must be ACTIVE to retire`,
        ],
      };
    }
