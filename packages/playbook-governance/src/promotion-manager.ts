...
export class PromotionManager {
  ...
  compareVersions(
    ...
    for (const stageId of commonStages) {
      ...
      const fromActions = fromStage.mustDo.map(a => a.actionId);
      const toActions = toStage.mustDo.map(a => a.actionId);

      const stageAddedActions = toActions.filter(a => !fromActions.includes(a));
      const stageRemovedActions = fromActions.filter(
        a => !toActions.includes(a)
      );

      addedActions.push(...stageAddedActions.map(a => `${stageId}:${a}`));
      removedActions.push(...stageRemovedActions.map(a => `${stageId}:${a}`));
      ...
      const commonActions = fromActions.filter(a => toActions.includes(a));
      for (const actionId of commonActions) {
        const fromAction = fromStage.mustDo.find(a => a.actionId === actionId);
        const toAction = toStage.mustDo.find(a => a.actionId === actionId);
