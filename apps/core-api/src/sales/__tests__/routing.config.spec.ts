...
describe('Routing Configuration Integration (Unit)', () => {
  ...
  beforeEach(async () => {
    ...
    leadRouter = module.get<LeadRouterService>(LeadRouterService);
    predictiveRouter = module.get<PredictiveRoutingService>(
      PredictiveRoutingService
    );
  ...
  describe('Lead Router - Geographic Preferences', () => {
    ...
    it('should fall back to defaults when no geographic preferences exist', async () => {
      ...
      // Should route to default global team
  ...
  describe('Predictive Router - Team Capacities', () => {
    it('should respect tenant-specific team capacity limits', async () => {
      ...
      // Mock team loads to test capacity limits
      const mockTeams = [
        {
          ...
          currentLoad: 10, // Below tenant A limit (12), at tenant B limit (would be ineligible)
          capacityLimit: 12, // Will be overridden by tenant config
          regions: ['north-america'],
          averageResponseTime: 15,
        },
      ];

      // For this test, we'll mock the teams to simulate the capacity logic
      // In real implementation, the service would use the configured capacities
    ...
    it('should apply different routing algorithms per tenant', async () => {
      ...
      // This would be verified by checking the factors in the results
  ...
  describe('Configuration Fallback Behavior', () => {
    it('should use defaults when configuration is missing', async () => {
      ...
      expect(routingResult?.routedTo).toBe('global-team');
    ...
    it('should use system tenant configuration as fallback', async () => {
      ...
      // Should use system tenant config
      expect(routingResult).toBeDefined();
      expect(predictiveResult).toBeDefined();
    });
  });
  ...
  describe('Tenant Isolation', () => {
    ...
    it('should prevent cross-tenant configuration leakage', async () => {
      ...
      // Should use defaults, not tenant A's config
      expect(routingResult?.routedTo).toBe('global-team'); // Default, not tenant A preference
      expect(predictiveResult).toBeDefined(); // Should still work with defaults
    });
  });
